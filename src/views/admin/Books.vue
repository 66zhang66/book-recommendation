    ...
    <div class="book-grid">
      <div v-for="book in books" :key="book.id" class="book-card">
        ...
        <div class="actions">
          ...
          <button class="delete-btn" @click="handleDelete(book.id)">删除</button>
        </div>
      </div>
    </div>
...
import { getBookList, addBook, updateBook, deleteBook, searchBooks } from '../../api/index.js'

const books = ref([])
const keyword = ref('')
const showDialog = ref(false)
const isEdit = ref(false)
const form = ref({ id: null, title: '', author: '', category: '', totalPages: 0 })
...
const openAddDialog = () => {
  form.value = { id: null, title: '', author: '', category: '', totalPages: 0 }
  isEdit.value = false
...
const save = async () => {
  ...
  try {
    let res
    if (isEdit.value) {
      res = await updateBook(form.value)
    } else {
      res = await addBook(form.value)
    }
    if (res.code === 200) {
